...
def extract_fibers_for_pair(pair, F, Nside, side_length, actual_row_idx):
    ...
    print(f"single IFU fiber number for row {actual_row_idx} = {len(fibers_tab_filtered)}")
...
# In[5]:
def main(row):
    input_file = 'filtered_pairs_table3.fits'
    ...
    for i, pair_row in enumerate(filtered_pairs):
        ...
        if median_spec is not None and err is not None:
            ...
            with open(f'lumpairs_between_single_IFU/lum_pairs_{actual_row_idx}.pkl', 'wb') as f:
                pickle.dump({
                    'lum': lum,
                    'rest_wl': rest_wl,
                    'lum_err': lum_err
